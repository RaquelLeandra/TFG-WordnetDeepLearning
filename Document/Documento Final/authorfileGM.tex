...
Partiendo de los resultados del apartado \ref{sec:synsets} hemos generado una distancia que representa lo diferentes que son cada uno de los \textit{synsets} que aparecen en \textit{imagenet} para la red convolucional entrenada. En este apartado explicaremos como se ha obtenido y que representa. 

\begin{definition}
Para empezar, tenemos el conjunto de 1,000 clases posibles en las que clasifica nuestra red convolucional, cada una correspondiente a un \textit{synset} al que llamaremos $\mathfrak{C}$.  
...
\begin{equ}[H]
\begin{equation*}
\mathfrak{C} \in \mathfrak{I} \in \mathfrak{W}
\end{equation*}
\end{equ}

Nuestro objetivo es construir una distancia para $\mathfrak{I}$ que dependa de la red convolucional entrenada.
Para ello primero notamos que la forma natural de conectar diferentes \textit{synsets} sería a partir de un árbol utilizando las conexiones de hiponimia e hiperonimia, como vimos en la figura \ref{fig:hiponimos}. Sin embargo, una distancia que utilizase solamente esta información no dependería de nuestra red. 

Para solventar este problema utilizamos la información obtenida en la sección \ref{sec:synsets}, específicamente la relacionada con la figura \ref{fig:comparativaCategoriasporSynset}. En ella vimos que utilizando los vectores representantes de cada \textit{synset} se podía distinguir lo concreto que era un \textit{synset} 
